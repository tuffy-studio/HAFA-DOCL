...
class TokenWise_TokenReducer(nn.Module):
    def __init__(self, input_dim=768, hidden_dim=128, temperature=2.0):
        super().__init__()
    ...
    def forward(self, x, return_weights=False, weights_entropy=False):
        ...
        if return_weights:
            return aggregated, token_weights
        elif weights_entropy:
            entropy = - (token_weights * torch.log(token_weights + 1e-8)).sum(dim=-1).mean()
            return aggregated, entropy
        else:   
            return aggregated

class MoE_Scorer(nn.Module):
    def __init__(self, input_dim=768, hidden_dim=128, num_experts=4):
        super().__init__()
    ...
    def forward(self, x):  # x: [B, D]
        ...
        # 最终加权输出: [B, 1]
        final_score = torch.sum(gate_weights * expert_outputs, dim=1, keepdim=True)

        return final_score

class TokenWise_TokenReducer_MoE(nn.Module):
    def __init__(self, input_dim=768, hidden_dim=128, num_tokens=6, num_experts=5):
        super().__init__()
        self.num_tokens = num_tokens
        self.token_moe_scorers = nn.ModuleList([
            MoE_Scorer(input_dim, hidden_dim, num_experts)
            for _ in range(num_tokens)
        ])

    def forward(self, x):  # x: [B, num_tokens, D]
        scores = []

        for i in range(self.num_tokens):
            token_i = x[:, i, :]  # [B, D]
            score_i = self.token_moe_scorers[i](token_i)
            scores.append(score_i)

        token_scores = torch.cat(scores, dim=1)  # [B, num_tokens]
        token_weights = F.softmax(token_scores, dim=-1)  # [B, num_tokens]
